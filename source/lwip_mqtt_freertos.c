...
#define SW3_TIMER_PERIOD_MS (1000 / portTICK_PERIOD_MS)
static void Sw3TimerCallback(TimerHandle_t xTimer);

#define SW2_TIMER_PERIOD_MS (1000 / portTICK_PERIOD_MS)
static void Sw2TimerCallback(TimerHandle_t xTimer);



volatile bool g_ButtonPress_3,g_ButtonPress_2 = false;
    ...
    In_Data[len] = 0;
    	if(strcmp(In_Data, "Rojo") == false){
    		...
    		if(strcmp(In_Data,"Verde") == false){
       		...
       		if(strcmp(In_Data,"Morado") == false){
       			GPIO_PortClear(BOARD_LED_RED_GPIO, 1u << BOARD_LED_RED_GPIO_PIN);
       			GPIO_PortClear(BOARD_LED_BLUE_GPIO, 1u << BOARD_LED_BLUE_GPIO_PIN);
      			GPIO_PortSet(BOARD_LED_GREEN_GPIO, 1u << BOARD_LED_GREEN_GPIO_PIN);
       	}
        else
       		if(strcmp(In_Data,"Blanco") == false){
       			GPIO_PortClear(BOARD_LED_RED_GPIO, 1u << BOARD_LED_RED_GPIO_PIN);
       			GPIO_PortClear(BOARD_LED_GREEN_GPIO, 1u << BOARD_LED_GREEN_GPIO_PIN);
       			GPIO_PortClear(BOARD_LED_BLUE_GPIO, 1u << BOARD_LED_BLUE_GPIO_PIN);
        }
        else
      		if(strcmp(In_Data,"OFF") == false){
      			GPIO_PortSet(BOARD_LED_RED_GPIO, 1u << BOARD_LED_RED_GPIO_PIN);
      			GPIO_PortSet(BOARD_LED_GREEN_GPIO, 1u << BOARD_LED_GREEN_GPIO_PIN);
      			GPIO_PortSet(BOARD_LED_BLUE_GPIO, 1u << BOARD_LED_BLUE_GPIO_PIN);
       	}
...
{
    static const char *topics[] = {"practica2/#"};
    int qos[]                   = {1};
    err_t err;
    int i;
...
{
    static const char *topic   = "practica2/rojo";
    static const char *message = "Rojo";
...
static void publish_message_OFF(void *ctx)
{
    static const char *topic   = "practica2/rojo";
...
static void app_thread(void *arg)
{
    ...
    TimerHandle_t Sw3TimerHandle = NULL;
    TimerHandle_t Sw2TimerHandle = NULL;

    /* Create the software timer. */
    Sw3TimerHandle = xTimerCreate("Sw3Timer",          /* Text name. */
                                 SW3_TIMER_PERIOD_MS, /* Timer period. */
                                 pdTRUE,             /* Enable auto reload. */
                                 0,                  /* ID is not used. */
                                 Sw3TimerCallback);   /* The callback function. */

    /* Create the software timer. */
    Sw2TimerHandle = xTimerCreate("Sw2Timer",          /* Text name. */
                                 SW2_TIMER_PERIOD_MS, /* Timer period. */
                                 pdTRUE,             /* Enable auto reload. */
                                 0,                  /* ID is not used. */
                                 Sw2TimerCallback);   /* The callback function. */
    ...
    xTimerStart(Sw3TimerHandle, 0);
    xTimerStart(Sw2TimerHandle, 0);
...
static void Sw3TimerCallback(TimerHandle_t xTimer)
{
    if (connected)
     {
    	if(g_ButtonPress_3 == true)
    	{
    		 tcpip_callback(publish_message, NULL);
    		 g_ButtonPress_3 = false;
    	}
      }
}
...
static void Sw2TimerCallback(TimerHandle_t xTimer)
{
    if (connected)
     {
    	if(g_ButtonPress_2 == true)
    	{
    		 tcpip_callback(publish_message_OFF, NULL);
